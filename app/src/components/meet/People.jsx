...
const People = ({ people, containerDimensions }) => {
  ...
  const gridStyle = containerDimensions ? getGridStyle(
    ...
    <View style={peopleStyles.container}>
      {
        visiblePeople?.map((person, index) => {
          return (
            <View
              key={index}
              style={[peopleStyles.card, person?.speaking ? {
                borderWidth: 3
              } : null, Array.isArray(gridStyle) ? gridStyle[index] : gridStyle]}
            >
              {
                person?.videoOn && person?.streamURL?.toURL() ? (
                ...
                ) : <View style={peopleStyles.noVideo}>
                  {
                    person?.photo ? (
                      ...
                      <Text style={peopleStyles.initial}>{person?.name?.chatAt(0)}</Text>
                    )
                  }
                </View>
              }
...
const getGridStyle = (count, containerWidth, containerHeight) => {
  ...
  switch (count) {
    ...
    default: {
      ...
      const itemHeight = containerHeight / maxRows - 40;
